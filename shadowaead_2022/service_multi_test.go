...
func TestMultiService(t *testing.T) {
	...
	multiService, err := shadowaead_2022.NewMultiService[string](method, iPSK[:], 500, &multiHandler{t, &wg}, nil, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	client, err := shadowaead_2022.New(method, [][]byte{iPSK[:], uPSK[:]}, nil, 0)
	if err != nil {
		t.Fatal(err)
	}
	wg.Add(1)
